# 7 dB-Hz known-bit deep hold - seed 20260716

## 测试目的

Validate the BDS B1I known-navigation-bit target with three deterministic noise seeds.

## 输入

- 信号：BDS B1I D1，PRN 6，7.68 Msps，零中频，3-bit I/Q。
- 数据链路：StarGen 实时二进制管道输入 StarTrack，不生成中间信号文件。
- 时长与种子：900 s，seed 20260716。
- C/N0：Step，0s:7dB-Hz。
...
- 状态路径：`Deep`。
- 末段多普勒 RMS / P95 / 最大绝对误差：0.4501 / 0.9586 / 1.5174 Hz。
- 末段码相位 MAE / P95 / 最大绝对误差：0.0429 / 0.0966 / 0.1212 chip。
- C/N0 偏差 / RMSE / 有效率：-1.166 / 2.350 dB / 0.973。
- 回捕次数：0；切换后发散次数：0。
- 仿真墙钟耗时：128.41 s，处理速度为实时的 7.01 倍。
